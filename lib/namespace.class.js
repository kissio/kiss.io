...
/**
 * Static function that used as helper to generate
 * ..an id for namespace.
 ...
 * @returns {Namespace} self
 */
Namespace.prototype.setFlag = function(key, value)
{
...
/**
 * Sets a local parameter for the namespace.
 *
 * @access public
 *
 * @param {String} key
...
/**
 * Gets a local parameter of the namespace.
 *
 * @access public
 *
 * @param {String} key
...
/**
 * Mounts a middleware/route/router on the namespace.
 *
 * @access public
 *
 ...
 * @return {Socket} the new socket created by the namespace
 */
        ...
        function bindHandler(i)
        {
          ...
          else
          {
            return fn.bind({
              nsp: self,
              socket: socket,
              next: bindHandler(i + 1)
            });
          }
        }

        socket.on(event, bindHandler(0));
      });
...
Namespace.prototype.broadcast = function(ev)
{
  ...
  for(var s of Object.keys(this.sockets))
  {
    if(except.indexOf(s.id) !== -1) continue;
